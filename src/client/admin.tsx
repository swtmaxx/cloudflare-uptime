...
import type { AdminSettings, CheckResult, Monitor, ProbeNode, StatusPage, Tag, User } from './types';
...
function AboutView({ settings, user }: { settings: AdminSettings; user: User }): JSX.Element {
  return <><PageHead kicker="About" title="关于" note="当前部署使用的运行时、探测服务和存储边界。" /><div className="about-grid"><div className="about-item"><strong>Pulseboard</strong><span>Cloudflare Worker 上的可用性监控控制台与公开状态页。</span></div><div className="about-item"><strong>Worker + D1</strong><span>单 Worker 提供 API、调度和页面；D1 保存监控、心跳、标签和状态页配置。</span></div><div className="about-item"><strong>Check-Host + Globalping</strong><span>支持 HTTP、TCP 端口和多地区 HTTP 检查，当前调度间隔为 1 分钟。</span></div></div><section className="section"><div className="panel"><div className="info-list"><div className="info-row"><span className="info-key">公开页面</span><span className="info-value">/status/&lt;slug&gt;</span></div><div className="info-row"><span className="info-key">RSS</span><span className="info-value">/status/&lt;slug&gt;/rss.xml</span></div><div className="info-row"><span className="info-key">当前账号</span><span className="info-value">{user.username}</span></div><div className="info-row"><span className="info-key">历史策略</span><span className="info-value">{settings.historyRetentionDays === 0 ? '永久保留' : `${settings.historyRetentionDays} 天`}</span></div></div></div></section></>;
}
...
function MonitorView({ settings, notify }: MonitorViewProps): JSX.Element {
  ...
  const load = async () => { try { const [monitorData, nodeData] = await Promise.all([api<{ monitors: Monitor[]; tags: Tag[] }>('/api/monitors?includeHistory=1'), api<{ nodes: ProbeNode[] }>('/api/nodes')]); setMonitors(monitorData.monitors); setTags(monitorData.tags || []); setNodes(nodeData.nodes); } catch (reason) { notify(reason instanceof Error ? reason.message : '无法读取监控', true); } };
  useEffect(() => { load(); }, []);
  ...
  const refreshNodes = async () => {
    ...
    try {
      const data = await api<{ count: number; nodes: ProbeNode[] }>('/api/nodes/refresh', { method: 'POST' });
      setNodes(data.nodes || []);
      notify(`已同步 ${data.count} 个 Check-Host 节点`);
    } catch (reason) {
      notify(reason instanceof Error ? reason.message : '同步节点失败', true);
    } finally {
      setRefreshingNodes(false);
    }
  };
  return <><PageHead kicker="Monitors" title="监控管理" note="每个监控每分钟发起一次多地区检查，可按监控选择 Check-Host 或 Globalping。" action={<div className="actions"><button className="button small ghost" type="button" onClick={refreshNodes} disabled={refreshingNodes}>{refreshingNodes ? '同步中…' : '同步节点'}</button><button className="button primary" type="button" onClick={() => { setEditing(null); setModal('monitor'); }}>＋ 新建监控</button></div>} /><div className="filter-bar"><input className="filter-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索名称、地址或标签" /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">全部状态</option><option value="up">正常</option><option value="degraded">部分异常</option><option value="down">宕机</option><option value="unknown">待探测</option><option value="paused">已暂停</option></select><select value={tagId} onChange={(event) => setTagId(event.target.value)}><option value="">全部标签</option>{tags.map((tag) => <option value={tag.id} key={tag.id}>{tag.name}</option>)}</select><button className="button small ghost" type="button" onClick={() => setModal('tags')}>管理标签</button></div><div className="panel table-wrap">{filtered.length ? <table><thead><tr><th>监控 / 最近心跳</th><th>类型</th><th>状态</th><th>探测地区</th><th>最近检查</th><th /></tr></thead><tbody>{filtered.map((monitor) => <tr key={monitor.id}><td><div className="monitor-name">{monitor.name}</div><div className="target">{monitor.targetUrl || `${monitor.host}:${monitor.port}`}</div><TagList tags={monitor.tags} /><div className="heartbeat-cell"><HeartbeatBar history={monitor.history} timeDisplay={settings.timeDisplay} /></div></td><td><span className="tag">{typeLabel(monitor.type)}</span> <span className="tag">{providerLabel(monitor.provider)}</span></td><td><StatusBadge status={monitor.enabled ? monitor.currentStatus : 'paused'} /></td><td>{monitor.provider === 'globalping' ? monitor.globalpingLocations.map((location) => <span className="tag" style={{ margin: 2 }} key={`${location.country}-${location.city}`}>{location.country} · {location.city || '全境'}</span>) : monitor.nodes.map((node) => <span className="tag" style={{ margin: 2 }} key={node.id}>{node.city}</span>)}</td><td>{formatAdminDate(monitor.lastCheckedAt, settings.timeDisplay)}</td><td><div className="actions"><button className="button small" type="button" onClick={() => action(`/api/monitors/${monitor.id}/check-now`, { method: 'POST' }, '检查任务已提交')}>立即检查</button><button className="button small ghost" type="button" onClick={() => { setEditing(monitor); setModal('monitor'); }}>编辑</button><button className="button small ghost" type="button" onClick={() => action(`/api/monitors/${monitor.id}`, { method: 'PATCH', body: JSON.stringify({ enabled: !monitor.enabled }) }, monitor.enabled ? '监控已暂停' : '监控已启用')}>{monitor.enabled ? '暂停' : '启用'}</button><button className="button small ghost" type="button" onClick={() => { if (window.confirm('清理这个监控的全部历史记录？')) action(`/api/monitors/${monitor.id}/history`, { method: 'DELETE' }, '历史记录已清理'); }}>清理历史</button><button className="button small ghost danger" type="button" onClick={() => { if (window.confirm('删除这个监控及其历史记录？')) action(`/api/monitors/${monitor.id}`, { method: 'DELETE' }, '监控已删除'); }}>删除</button></div></td></tr>)}</tbody></table> : <Empty title="没有匹配的监控" note="调整搜索或筛选条件。" />}</div>{modal === 'monitor' ? <MonitorForm monitor={editing} nodes={nodes} tags={tags} notify={notify} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} /> : null}{modal === 'tags' ? <TagManager tags={tags} setTags={setTags} notify={notify} onClose={() => setModal(null)} /> : null}</>;
}
